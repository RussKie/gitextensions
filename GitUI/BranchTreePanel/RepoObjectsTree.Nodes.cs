...
namespace GitUI.BranchTreePanel
{
    partial class RepoObjectsTree
    {
        private sealed class Nodes : IEnumerable<Node>
        {
            ...
            /// <summary>
            /// Adds a new node to the collection ensuring that the new node will be added
            /// before a "Hidden" node, if the collection contains it.
            /// </summary>
            /// <param name="node">The node to add.</param>
            public void AddNodeBeforeHidden(Node node)
            {
                if (LastNode is BaseBranchNode lastNode && lastNode.Name == Strings.Hidden)
                {
                    // Ensure the "Hidden" node is the last node in the tree
                    InsertNode(Count - 1, node);
                    return;
                }
        ...
        private abstract class Tree : IDisposable
        {
            ...
            private protected static AsyncLocal<bool> IsApplyFiltering = new AsyncLocal<bool>();
